...
trigger EventBefore on Event (before insert, before update) {
    ...
    for (Event evt : Trigger.new) {
        String meetingCompletedState = 'Meeting Completed';
        // Default meeting confirmed
        if(evt.Send_Email_Alert__c == false && meetingCompletedState != evt.Event_Status__c ){
        ...
        if (EventBeforeSupport.B2B_EVENT_RT == evt.RecordTypeId && null != evt.StartDateTime){ 
            ...
            if (null != evt.WhatId) {
                ...
                opportunityById.put(evt.WhatId, null);
            }
            if (null != evt.OwnerId) {
    ...
    //load accounts
        ...
        opportunityById = new Map<Id, Opportunity> ([SELECT Id, Name, AccountId, Account.Name
        											 FROM Opportunity
    ...
    for (Event evt : Trigger.new) {
	    if (EventBeforeSupport.B2B_EVENT_RT == evt.RecordTypeId && null != evt.StartDateTime) { // Event is in the future
	        ...
	        if (brandName == null && opportunityById.containsKey(evt.WhatId)) {
	        	brandName = opportunityById.get(evt.WhatId).Account.Name;
	        }
